...
def run_eda():
    ...
    input_file = os.path.join("..", "data", "data_raw", "store_sales_holidays_events.csv")
    ...
    summary = []
    summary.append(f"=== RELATÓRIO DE EDA: store_sales_holidays_events.csv ===")
    ...
    report_name = "eda_report_store_sales_holidays_events.txt"
    ...
    if date_cols and len(num_cols) > 0:
        ...
        plt.title(f"Série Temporal: {target_col} vs {date_col} (store_sales_holidays_events.csv)", fontsize=14, fontweight='bold', pad=15)
        plt.xlabel(date_col, fontsize=12)
        plt.ylabel(target_col, fontsize=12)
        plt.grid(True, linestyle='--', alpha=0.6)
        plt.legend()
        
        chart_name = "eda_chart_store_sales_holidays_events.png"
        ...
        plt.title(f"Distribuição de {target_col} (store_sales_holidays_events.csv)", fontsize=14, fontweight='bold', pad=15)
