...
def run_eda():
    ...
    input_file = os.path.join("..", "data", "data_raw", "mmm_walentosky_bike_sales_data.csv")
    ...
    summary = []
    summary.append(f"=== RELATÓRIO DE EDA: mmm_walentosky_bike_sales_data.csv ===")
    ...
    report_name = "eda_report_mmm_walentosky_bike_sales_data.txt"
    ...
    if date_cols and len(num_cols) > 0:
        ...
        plt.title(f"Série Temporal: {target_col} vs {date_col} (mmm_walentosky_bike_sales_data.csv)", fontsize=14, fontweight='bold', pad=15)
        plt.xlabel(date_col, fontsize=12)
        plt.ylabel(target_col, fontsize=12)
        plt.grid(True, linestyle='--', alpha=0.6)
        plt.legend()
        
        chart_name = "eda_chart_mmm_walentosky_bike_sales_data.png"
        ...
        plt.title(f"Distribuição de {target_col} (mmm_walentosky_bike_sales_data.csv)", fontsize=14, fontweight='bold', pad=15)
