...
def run_eda():
    ...
    input_file = os.path.join("..", "data", "data_raw", "processed_data.csv")
    ...
    summary = []
    summary.append(f"=== RELATÓRIO DE EDA: processed_data.csv ===")
    ...
    report_name = "eda_report_processed_data.txt"
    ...
    if date_cols and len(num_cols) > 0:
        ...
        plt.title(f"Série Temporal: {target_col} vs {date_col} (processed_data.csv)", fontsize=14, fontweight='bold', pad=15)
        plt.xlabel(date_col, fontsize=12)
        plt.ylabel(target_col, fontsize=12)
        plt.grid(True, linestyle='--', alpha=0.6)
        plt.legend()
        
        chart_name = "eda_chart_processed_data.png"
        ...
        plt.title(f"Distribuição de {target_col} (processed_data.csv)", fontsize=14, fontweight='bold', pad=15)
