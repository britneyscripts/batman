...
def run_eda():
    ...
    input_file = os.path.join("..", "data", "data_raw", "energy_DOM_hourly.csv")
    ...
    summary = []
    summary.append(f"=== RELATÓRIO DE EDA: energy_DOM_hourly.csv ===")
    ...
    report_name = "eda_report_energy_DOM_hourly.txt"
    ...
    if date_cols and len(num_cols) > 0:
        ...
        plt.title(f"Série Temporal: {target_col} vs {date_col} (energy_DOM_hourly.csv)", fontsize=14, fontweight='bold', pad=15)
        plt.xlabel(date_col, fontsize=12)
        plt.ylabel(target_col, fontsize=12)
        plt.grid(True, linestyle='--', alpha=0.6)
        plt.legend()
        
        chart_name = "eda_chart_energy_DOM_hourly.png"
        ...
        plt.title(f"Distribuição de {target_col} (energy_DOM_hourly.csv)", fontsize=14, fontweight='bold', pad=15)
